//填写自己的鉴权服务器地址
// 多次
// const cosSignatureUrl_duoci = 'http://meishi.com/api/cos/qianmingduoci'
// 单次
const cosSignatureUrl_danci = 'http://meishi.com/api/cos/qianmingdanci'

class Cos {
  constructor() { }


  // 多次签名不限制COS桶下的文件夹路径，可用于上传文件方便. 
  // 单次签名必须要有文件路径，用于更新文件信息或删除文件.

  // 获取多次签名(有过期时间) -> 请求签名后缓存，以备后续使用
  // qianmingDuoci() {
  //   wx.request({
  //     url: cosSignatureUrl_duoci,
  //     method: 'POST',
  //     success(res) {
  //       console.log('qianmingDuoci', res.data)
  //       wx.setStorageSync('qianming', res.data)
  //     }
  //   })
  // }

  // 获取单次签名
  qianmingDanci(dir, callback) {
    wx.request({
      url: cosSignatureUrl_danci,
      method: 'POST',
      data: { filepath: dir },  // 服务器那边需要一个路径.post送去
      success(res) {
        console.log('qianmingDanci', res.data)
  ...
  // 上传图片 -》 dir=文件夹  '/a' /开头
  upload(dir, filePath, fileName, callback) {

    let cosUrl = "https://" + 'cd' + ".file.myqcloud.com/files/v2/" + '1253443226' + "/" + 'cosceshi' + dir

    this.qianmingDanci(dir, res => {
      // 头部带上签名，上传文件至COS
      wx.uploadFile({
        url: cosUrl + '/' + fileName,
        filePath: filePath,
        ...
        name: 'filecontent',
        formData: { op: 'upload', insertOnly: 0 },  // 必须.insertOnly.重名文件覆盖
        success: function (uploadRes) {
          ...
          if (data.code == 0) {
            ...
            console.log('Cos上传文件code不等于0,来自Cos类upload方法', data)
            if (data.code == -177) { console.log('文件已存在,来自Cos类upload方法') }
          }
          //do something
        },
        fail: function (e) { console.log('Cos上传文件进入fail,来自Cos类upload方法', e) }
      })
    })
  ...
  // 删除图片
  deleteCos(dir, callback) {
    wx.request({
      url: 'http://meishi.com/api/cos/delete',
      method: 'POST',
      data: { path: dir },  // "/canting/8-1.jpg"
